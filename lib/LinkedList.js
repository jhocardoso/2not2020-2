...
module.exports = class LinkedList {
    ...
    push(value) {
        ...
        // Caso especial: a lista está vazi
        if(this.count === 0) {
            this.head = node
        }
        else {
    ...
    insertAt(position, value) {
        ...
        if(position >= 0 && position <= this.count) {
            ...
            // Caso especial: inserção na última posição
                ...
                node.next = nodePos
                // O node anterior passa a apontar para o novo nodo
                nodePrev.next = node
            }
            this.count++
        }
    ...
    pop() {
        if(this.count > 0) {
            ...
            // Caso especial: retirada do último elemento da lista
            if(this.count === 1) {
                removed = this.head
                this.head = null
            }
    ...
    // Retorna o elemento na psição especificada, sem retirar da lista
    getAt(position) {
        if(position >= 0 && position < this.count) {
